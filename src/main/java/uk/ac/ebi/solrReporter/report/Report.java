...
public class Report {
    ...
    public boolean generateReport(ReportData data) {
        ...
        try {
            writer = new PrintWriter("report_" + dateFormat.format(cal.getTime()) + ".txt");
        } catch (FileNotFoundException e) {
            log.error("Error creating report txt file", e);
            return false;
        }

        writer.println("- - - - - - - - - - - - Solr Indexer Report - - - - - - - - - - - -");
        // GROUPS
    ...
    public Set<String> getMissingInIndex(Set<String> dbAcc, Set<String> solrAcc) {
        ...
        if(dbAcc != null && solrAcc != null) {
            missingInIndex = dbAcc.stream().filter(db -> !solrAcc.contains(db)).collect(Collectors.toSet());
        }
    ...
    public Set<String> getPresentInIndex(Set<String> dbAcc, Set<String> solrAcc) {
        ...
        if(dbAcc != null && solrAcc != null) {
            presentInIndex = solrAcc.stream().filter(solr -> !dbAcc.contains(solr)).collect(Collectors.toSet());
        }
